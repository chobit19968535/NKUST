# -*- coding: utf-8 -*-
"""
Created on Thu Mar  5 19:22:19 2020
...
parser.add_argument('--classes_file', type=str, help='The .txt file include dataset <classes>',
                    default='model_data/classes.txt')
...
class Yolo(object):
    def __init__(self,weights_path):
        ...
        self.input_shape = (320, 320)  # multiple of 32, hw
        self.shape = (320, 320, 3)

        # training batch size
        self.step1_batch_size = 32
        self.step2_batch_size = 4  # note that more GPU memory is required after unfreezing the body
        self.yolo_model, self.yolo_body = self.create_model(yolo_weights_path = self.yolo_weights_path)
    ...
    def create_model(self, load_pretrained=True, freeze_body=2,
        ...
        model_loss = Lambda(yolo_loss, output_shape=(1,), name='yolo_loss',
                            arguments={'anchors': self.anchors, 'num_classes': self.num_classes, 'ignore_thresh': iou_loss})(
...
#                ["False", 60, 0.8, 0.4, 0.5, 1.5, 0.7, "005"],  #18 
                 ["True", 40, 0.55, 0.1, 1.5, 0.5, 0.3, "005"],  #Best_confirmExp
                ]
    for index, val in enumerate(exp_list):
        flip, max_boxes, jitter, hue, sat, val, iou_loss, leaky = val
        index +=1 # shift exps
#        log_dir_tmp = 'logs/320_uniform_exp' + str(index)
        log_dir_tmp = 'BCCD_logs//320_uniform_Best_Best' + str(index)
        weights_path = 'Models/leaky' + leaky + '_320.h5'

        for j in range(3):
            log_dir = log_dir_tmp + '_y' + str(j+7) + '/'
#            weights_path = log_dir_tmp + '_y' + str(j+1) + '/models/Step2_yolo_weight_mAP_best.h5'
#            log_dir = log_dir_tmp + '_y' + str(j+1) + '/Stage2_add15/'
            LOGS_PATH = log_dir
            MODELS_PATH = os.path.join(LOGS_PATH, 'models')
            if not os.path.exists(MODELS_PATH):
                os.makedirs(MODELS_PATH)
                
            yolo = Yolo(weights_path = weights_path)
            try:
                yolo.train()
